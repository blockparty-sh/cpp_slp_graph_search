...
int main(int argc, char * argv[])
{
    ...
    if (transactionType == "GENESIS") {
        ...
        ABORT_CHECK (j["symbol"].get<std::string>() != hex(slp.ticker));
        ABORT_CHECK (j["name"].get<std::string>() != hex(slp.name));

        std::cout << j["documentUri"].get<std::string>() << std::endl;
        ABORT_CHECK (j["documentUri"].get<std::string>() != hex(slp.document_uri));
