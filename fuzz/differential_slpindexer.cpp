...
int main(int argc, char * argv[])
{
    ...
    httplib::Client cli("127.0.0.1", 7331);
    std::string path = "/api/v1/fuzz/decodeSlp/"+hex(txdata);
    ...
    if (jbody["transactionType"].is_null()) {
        exit_code = 1;
    }


    // hairy - we look to see if true != 0 and likewise false != 1..
    ABORT_CHECK (hydration_success && !!exit_code && "c++ parsed, slp-indexer did not");
    ABORT_CHECK (! hydration_success && !exit_code && "c++ did not parse, but slp-indexer did");
    ...
    if (transactionType == "GENESIS") {
        ...
        /*
         * disabled because slp-indexer does not support documentSha256
